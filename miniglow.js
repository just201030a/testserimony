...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".mini-info").forEach(info => {

    // إذا موجود لا يكرر
    if(info.querySelector(".original-product")) return;

    const original = document.createElement("p");
    original.className = "original-product";
    original.innerText = "100% ORIGINAL PRODUCT";

    // نحطه بعد اسم المنتج مباشرة
    const name = info.querySelector(".mini-name");
    name.insertAdjacentElement("afterend", original);
